...
export interface UserState {
  ...
  grade: string;
  exp: number;
  studyMinutes: number; // today
  studyDate: string; // YYYY-MM-DD
  levelStatuses: LevelStatus[]; // 5 levels
  unitsCompleted: number[][]; // [level][unit] count of modules done (0-3)
  loggedIn: boolean;
}
...
const defaultState: UserState = {
  ...
  studyMinutes: 0,
  studyDate: todayStr(),
  levelStatuses: ["unlocked", "locked", "locked", "locked", "locked"],
...
export function StoreProvider({ children }: { children: ReactNode }) {
  ...
  const persist = (u: UserState) => {
    setUser(u);
    if (typeof window !== "undefined") localStorage.setItem("mes_user", JSON.stringify(u));
  };
  ...
  const logout = () => {
    ...
    if (typeof window !== "undefined") localStorage.removeItem("mes_user");
  };
  ...
  const addStudyMinutes = (m: number) => {
    persist({ ...user, studyMinutes: user.studyMinutes + m, studyDate: todayStr() });
  };
  const setLevelStatus = (idx: number, s: LevelStatus) => {
    const arr = [...user.levelStatuses];
    arr[idx] = s;
    if (s === "completed" && idx + 1 < arr.length && arr[idx + 1] === "locked") {
      arr[idx + 1] = "unlocked";
    }
  ...
  const setPlacementLevel = (level: number) => {
    const arr: LevelStatus[] = ["locked", "locked", "locked", "locked", "locked"];
  ...
  return (
    <Ctx.Provider
      value={{ user, login, logout, addExp, addStudyMinutes, setLevelStatus, completeModule, setPlacementLevel }}
    >
      {children}
    </Ctx.Provider>
  );
